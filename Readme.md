...
## Before you push: this data contains PII

The WTS export has an unnamed column holding **recipient names**. `.gitignore`
excludes `packagestats.csv`, `packagestats.xlsx`, and
`files/packagestats_cleaned.xlsx` for that reason.

If those files were committed before `.gitignore` was added, they are still in
the git history and still readable on GitHub. Removing them needs a history
rewrite (`git filter-repo`, or a fresh repo), not just a delete-and-commit.
Check with:

```
git ls-files | findstr packagestats
```
...
**`elevenLabs.py`**: Text-to-speech scratch file for the spoken weekly
briefing. Not yet wired into the pipeline.
...
| `week_plan.csv` | `predict` | The Power BI source table |
...
**3. Saturdays are still under-predicted.** `saturday_in_break` raised the q80
band materially - the three previously uncovered Saturdays went from q80
26/27/15 against actuals 57/44/24 to 40/34/20 - but Saturday coverage is
**0.538** against a 0.80 target while every other weekday now sits between
0.77 and 0.88:

```
Friday 0.824   Monday 0.827   Saturday 0.538
Thursday 0.878   Tuesday 0.769   Wednesday 0.840
```

Saturdays remain the one weekday systematically missed low. **Fix:** fit
Saturdays as a separate level per period rather than as a coefficient on a
shared day-of-week term, or cap `sat_trend` so the declining-hours trend
cannot extrapolate past the observed floor.
...
1. **Saturdays** (problem 3). Coverage is 0.538 against 0.80. Fit Saturdays as
   a separate level per period, or cap `sat_trend` so the declining-hours
   trend cannot extrapolate past the observed floor.
...
`outputs/week_plan.csv` is the source table: one row per mailroom per day per
scenario, with `intake`, `intake_q50`, `occupancy_hat`, `pickups_hat`,
`disposals_hat`, `total_hours`, `assign_fte`, `lockers`,
`locker_utilisation_pct`, `current_headcount_fte`, `is_open`,
`closed_reason`, `source`, and `model_used`.

1. **Get** > **Text/CSV** > `outputs/week_plan.csv`. Set `date` and
   `week_start` to Date type, `scenario` and `site` to Text.
2. Add a **slicer on `scenario`** (`current` / `disposal_30d`). This is the
   disposal toggle. Both scenarios are precomputed nightly, so the toggle is
   pure filtering — Power BI cannot pass parameters back to Python, and
   precomputing is what makes the toggle instant.
3. Add a **slicer on `site`** for per-mailroom drill-down.
4. Core visuals:
   - Clustered column of `intake` by `day_name`, coloured by `source`, so
     actual and forecast days are visually distinct
   - Line of `occupancy_hat` by `date` with a constant line at `lockers`
   - Matrix of `site` x `day_name` showing `assign_fte`
   - Cards for weekly `total_hours` and peak `locker_utilisation_pct`
5. Sort `day_name` correctly by adding a day-index column in Power Query
   (Power BI sorts text alphabetically otherwise — Friday first).
6. For the accuracy page, load `test_predictions.csv` as a second table:
   actual vs predicted line chart with a q80 band, plus `test_accuracy.csv`
   as a table visual.
7. Refresh: point Power BI at the folder and use scheduled refresh. Run
   `python -m panther.run predict` nightly to regenerate the CSVs. Re-run
   `train` only when you want the models refitted; the winner rarely changes
   week to week.

### Spoken weekly briefing (ElevenLabs)

Power BI cannot call an external API from a button, so pre-generate the audio
in the nightly job and have the dashboard button open the file. Generate the
narration text from a template with the numbers substituted in, rather than
having a model write it freely, so what gets spoken is auditable.

Lead with labour and holdings, not raw intake:

> "Week of October 25 at Tower B: 2,137 parcels held against 611 lockers, with
> 3.5 staff recommended Wednesday through Friday as Halloween volume peaks."